...
namespace EliasFanoPackedBufferNS {
...
class EliasFanoPackedBuffer {
public:
    explicit EliasFanoPackedBuffer(uint32_t universe_maximum)
    ...
    void copy_elements_to_vector(std::vector<uint32_t> &res) {
        ...
        std::vector<uint32_t> buffer_vec;
        buffer_.copy_elements_to_vector(buffer_vec);

        for (int i = 0; i < buffer_vec.size(); ++i) {
            assert(buffer_vec[i] <= UINT32_MAX);
            res.push_back((uint32_t)buffer_vec[i]);
        }
}
...
private:
    ...
    void flush_buffer() {
        assert((int)buffer_.get_elem_count() == block_capacity_);
        int element_count = block_capacity_;
        std::vector<uint32_t> buffer_vec;
        buffer_.copy_elements_to_vector(buffer_vec);
        uint32_t base_value = buffer_vec[0];
        uint64_t universe_block_size = uint64_t(buffer_vec[element_count - 1]) - base_value + 1;
        ...
        for (int current_index = 0; current_index < element_count; ++current_index) {
            uint32_t relative_value = buffer_vec[current_index] - base_value;
