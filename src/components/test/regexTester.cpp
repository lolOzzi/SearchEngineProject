...
std::unordered_set<std::string> setIntersection(std::unordered_set<std::string>& set1, const std::unordered_set<std::string>& set2) {
    std::unordered_set<std::string> res;
    if (set1.size() >= set2.size()) {
        for (auto& s : set2) {
            if (set1.contains(s)) {
...
int main(int argc, char* argv[]) {
    ...
    State* start = createNFA(pf, nodes);
    bool res = match(start, q, nodes);
    ...
    auto ast = trigramTree(regex);
    //std::unordered_set<std::string> trigrams = extractTrigrams();
