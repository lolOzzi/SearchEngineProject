...
namespace BurstTrieDeltaDynamicStoreRegexNS {
    ...
    inline BurstTrieDeltaDynamicStoreRegex::BurstTrieDeltaDynamicStoreRegex()
        : word_dictionary(),
          postings_lists(),
          documents(2),
          last_added_document_id(0) {}

    inline void BurstTrieDeltaDynamicStoreRegex::add_document(Doc document) {
    ...
    inline void BurstTrieDeltaDynamicStoreRegex::add(std::string word, Doc document) {
        if (word.empty()) {
            return;
        }
    ...
    inline std::vector<Doc> BurstTrieDeltaDynamicStoreRegex::get(std::string word) {
    ...
    inline int BurstTrieDeltaDynamicStoreRegex::get_num_docs() {
        return documents.n;
    }

    inline Doc BurstTrieDeltaDynamicStoreRegex::build_document_from_id(uint32_t document_id) {
        Doc document;
        document.title = documents[document_id].title;
        document.start_loc = documents[document_id].start_location;
        return document;
    }
    inline std::vector<std::string> BurstTrieDeltaDynamicStoreRegex::getWordsFromTrigram(std::string tri) {
