...
namespace  DynamicFKSRadixTreeRegex {
    ...
    private:
        ...
        std::map<std::string, std::unordered_set<std::string>> trigram;
    ...
    public:
        ...
        std::unordered_set<std::string> getWordsFromTrigram(std::string tri);
    };

    inline DynamicFKS::DynamicFKS(int n, IHash* hash_function) {
        numBuckets = n;
    ...
    inline void DynamicFKS::add_document(Doc document) {
        last_document_node_added = doc_holder.add(document.title);
        last_document_title_added = document.title;
        documents_added++;
    }

    inline void DynamicFKS::trigramAdd(std::string& word) {
        for (int i = 0; i < word.length() - 2; i++) {
            std::string tri = word.substr(i, 3);
            trigram[tri].insert(word);
        }
    }

    inline void DynamicFKS::add(std::string word, Doc document) {
        if (word == "") return;
        if (word.length() >= 3) {
            trigramAdd(word);
        }
    ...
    inline std::unordered_set<std::string> DynamicFKS::getWordsFromTrigram(std::string tri) {
    ...
    inline std::vector<Doc> DynamicFKS::get(std::string word) {
    ...
    inline CollisionFree::CollisionFree(int size, RadixTree* tree) {
        this->n = 0;
        this->size = size;
        arr_size = size*size;
...
inline void CollisionFree::new_hash() {
    hasher.init();
}

    inline void CollisionFree::DoNew(DictWord* new_word, int old_arr_size) {
    ...
    inline DictWord* CollisionFree::add(std::string& word) {
        n++;
    ...
    inline DictWord* CollisionFree::get(std::string& word) {
        auto index = hasher.hash(word, arr_size);
    ...
    inline int DynamicFKS::get_num_docs() {
