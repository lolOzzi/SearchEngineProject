...
#include "../stores/RegexStoreComp.hpp"
...
namespace BurstTrieEliasFanoDynamicStoreRegexNS {
    ...
    class BurstTrieEliasFanoDynamicStoreRegex : public IStore {
    private:
    ...
    public:
        BurstTrieEliasFanoDynamicStoreRegex();
        ~BurstTrieEliasFanoDynamicStoreRegex() override = default;
    ...
    inline BurstTrieEliasFanoDynamicStoreRegex::BurstTrieEliasFanoDynamicStoreRegex()
        : word_dictionary(),
          postings_lists(),
          documents(2),
          last_added_document_id(0) {}

    inline void BurstTrieEliasFanoDynamicStoreRegex::add_document(Doc document) {
    ...
    inline void BurstTrieEliasFanoDynamicStoreRegex::add(std::string word, Doc document) {
        if (word.empty()) {
            return;
        }
    ...
    inline std::vector<Doc> BurstTrieEliasFanoDynamicStoreRegex::get(std::string word) {
    ...
    inline int BurstTrieEliasFanoDynamicStoreRegex::get_num_docs() {
        return documents.n;
    }

    inline Doc BurstTrieEliasFanoDynamicStoreRegex::build_document_from_id(uint32_t document_id) {
        Doc document;
        document.title = documents[document_id].title;
        document.start_loc = documents[document_id].start_location;
        return document;
    }
    inline std::vector<std::string> BurstTrieEliasFanoDynamicStoreRegex::getWordsFromTrigram(std::string tri) {
