...
namespace BurstTrieEliasFanoStaticStoreNS {
    ...
    inline BurstTrieEliasFanoStaticStore::BurstTrieEliasFanoStaticStore()
        : word_dictionary(),
          postings_lists(),
          documents(2),
          last_added_document_id(0) {}

    inline void BurstTrieEliasFanoStaticStore::add_document(Doc document) {
    ...
    inline void BurstTrieEliasFanoStaticStore::add(std::string word, Doc document) {
        if (word.empty()) {
            return;
        }
    ...
    inline std::vector<Doc> BurstTrieEliasFanoStaticStore::get(std::string word) {
    ...
    inline void BurstTrieEliasFanoStaticStore::compress() {
    ...
    inline int BurstTrieEliasFanoStaticStore::get_num_docs() {
        return documents.n;
    }

    inline Doc BurstTrieEliasFanoStaticStore::build_document_from_id(uint32_t document_id) {
        Doc document;
