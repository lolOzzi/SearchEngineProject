...
namespace RecSplitEliasFanoStaticStoreNS {
    ...
    inline RecSplitEliasFanoStaticStore::RecSplitEliasFanoStaticStore()
        : word_dictionary(4, &hash_family),
          postings_lists(),
          documents(2),
          last_added_document_id(0) {}

    inline void RecSplitEliasFanoStaticStore::add_document(Doc document) {
    ...
    inline void RecSplitEliasFanoStaticStore::add(std::string word, Doc document) {
        if (word.empty()) {
            return;
        }
    ...
    inline std::vector<Doc> RecSplitEliasFanoStaticStore::get(std::string word) {
    ...
    inline void RecSplitEliasFanoStaticStore::compress() {
    ...
    inline int RecSplitEliasFanoStaticStore::get_num_docs() {
        return documents.n;
    }

    inline Doc RecSplitEliasFanoStaticStore::build_document_from_id(uint32_t document_id) {
        Doc document;
