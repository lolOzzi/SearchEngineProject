...
namespace fuzzy {
    ...
    inline FuzzyDynamicFKS::FuzzyDynamicFKS(int n, IHash* hash_function) {
        numBuckets = n;
    ...
    inline void FuzzyDynamicFKS::add_document(Doc document) {
        last_document_node_added = doc_holder.add(document.title);
        last_document_title_added = document.title;
        documents_added++;
    }
    inline void FuzzyDynamicFKS::add(std::string word, Doc document) {
    ...
    inline FuzzyDictWord* FuzzyDynamicFKS::add_word_with_last_doc(std::string word) {
    ...
    inline FuzzyDictWord* FuzzyDynamicFKS::add_word(std::string word) {
    ...
    inline std::vector<Doc> FuzzyDynamicFKS::get_single(std::string word) {
    ...
    inline std::vector<Doc> FuzzyDynamicFKS::get(std::string word) {
        std::vector<Doc> res;
    ...
    inline StringCollisionFree::StringCollisionFree(int size, RadixTree* tree) {
        this->n = 0;
        this->size = size;
        arr_size = size*size;
    ...
    inline void StringCollisionFree::new_SimpleFingerprinthash() {
        hasher.init();
    }

    inline void StringCollisionFree::DoNew(FuzzyDictWord* new_word, int old_arr_size) {
    ...
    inline FuzzyDictWord* StringCollisionFree::add(std::string& word) {
        n++;
    ...
    inline FuzzyDictWord* StringCollisionFree::get(std::string& word) {
        auto index = hasher.hash(word, arr_size);
    ...
    inline int FuzzyDynamicFKS::get_num_docs() {
        return documents_added;
    }
    inline std::vector<std::string> FuzzyDynamicFKS::create_variants(std::string str, int max_distance) {
