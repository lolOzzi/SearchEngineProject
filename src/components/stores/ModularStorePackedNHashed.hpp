...
namespace ModularStorePackedNHashedNS {

    class ModularStorePackedNHashedDictWord {
    public:
        HashTrie::Node* word;
        uint32_t last_added_id;
        DynamicPackedArray documents_in;
        std::vector<ModularStorePackedNHashedDictWord*> originals;

        ModularStorePackedNHashedDictWord(){
            this->word = nullptr;
            documents_in = DynamicPackedArray{};
            last_added_id = 0;
            originals = std::vector<ModularStorePackedNHashedDictWord*>{}; // Fuzzy search not actually implemented
        }
        explicit ModularStorePackedNHashedDictWord(HashTrie::Node* node) {
            this->word = node; 
            documents_in = DynamicPackedArray{};
            last_added_id = 0;
            originals = std::vector<ModularStorePackedNHashedDictWord*>{};
        }
    };

    class ModularStorePackedNHashed : public IStore {
    private:
        // Words and titles yk
        ...
        CuckooHashingDictionary<std::string, ModularStorePackedNHashedDictWord> word_node_store;


    public:
        ModularStorePackedNHashed(StringHashFamily* _hasher, int size) : tree(), doc_nodes(100), word_node_store(size, _hasher) {hasher = _hasher;};
        ~ModularStorePackedNHashed() override = default;
    ...
    inline void ModularStorePackedNHashed::add_document(Doc document) {
    ...
    inline void ModularStorePackedNHashed::add(std::string word, Doc document) {
		auto word_node = tree.add(word);
    	ModularStorePackedNHashedDictWord* existing = word_node_store.get(word);
    	if (existing) {
    		...
    		ModularStorePackedNHashedDictWord pdw(word_node);
    		pdw.documents_in.add(id_for_last_added_doc);
            pdw.last_added_id = id_for_last_added_doc;
    		word_node_store.add(word, std::move(pdw));
    	}
    }
    inline std::vector<Doc> ModularStorePackedNHashed::get(std::string word) {
		ModularStorePackedNHashedDictWord* tdw = word_node_store.get(word);
    	if (!tdw) return {};
    ...
    inline void ModularStorePackedNHashed::map_id_to_doc_nodes(std::vector<Doc> &res, std::vector<uint32_t> &ids) {
    ...
    inline int ModularStorePackedNHashed::get_num_docs() {
      return doc_nodes.n;
    }
}
