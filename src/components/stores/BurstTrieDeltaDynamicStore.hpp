...
namespace BurstTrieDeltaDynamicStoreNS {
    ...
    inline BurstTrieDeltaDynamicStore::BurstTrieDeltaDynamicStore()
        : word_dictionary(),
          postings_lists(),
          documents(2),
          last_added_document_id(0) {}

    inline void BurstTrieDeltaDynamicStore::add_document(Doc document) {
    ...
    inline void BurstTrieDeltaDynamicStore::add(std::string word, Doc document) {
        if (word.empty()) {
            return;
        }
    ...
    inline std::vector<Doc> BurstTrieDeltaDynamicStore::get(std::string word) {
    ...
    inline int BurstTrieDeltaDynamicStore::get_num_docs() {
        return documents.n;
    }

    inline Doc BurstTrieDeltaDynamicStore::build_document_from_id(uint32_t document_id) {
        Doc document;
