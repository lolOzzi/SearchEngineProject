...
namespace BurstTrieEliasFanoDynamicStoreNS {
    ...
    inline BurstTrieEliasFanoDynamicStore::BurstTrieEliasFanoDynamicStore()
        : word_dictionary(),
          postings_lists(),
          documents(2),
          last_added_document_id(0) {}

    inline void BurstTrieEliasFanoDynamicStore::add_document(Doc document) {
    ...
    inline void BurstTrieEliasFanoDynamicStore::add(std::string word, Doc document) {
        if (word.empty()) {
            return;
        }
    ...
    inline std::vector<Doc> BurstTrieEliasFanoDynamicStore::get(std::string word) {
    ...
    inline int BurstTrieEliasFanoDynamicStore::get_num_docs() {
        return documents.n;
    }

    inline Doc BurstTrieEliasFanoDynamicStore::build_document_from_id(uint32_t document_id) {
        Doc document;
