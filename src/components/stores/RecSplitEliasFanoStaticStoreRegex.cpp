#include "RecSplitEliasFanoStaticStoreRegex.h"

namespace RecSplitEliasFanoStaticStoreNSRegex {
    RecSplitEliasFanoStaticStoreRegex::RecSplitEliasFanoStaticStoreRegex()
        : word_dictionary(4, &hash_family),
          postings_lists(),
          documents(2),
          last_added_document_id(0) {}

    void RecSplitEliasFanoStaticStoreRegex::add_document(Doc document) {
    ...
    void RecSplitEliasFanoStaticStoreRegex::add(std::string word, Doc document) {
        if (word.empty()) {
            return;
        }
    ...
    std::vector<Doc> RecSplitEliasFanoStaticStoreRegex::get(std::string word) {
    ...
    void RecSplitEliasFanoStaticStoreRegex::compress() {
    ...
    int RecSplitEliasFanoStaticStoreRegex::get_num_docs() {
        return documents.n;
    }

    Doc RecSplitEliasFanoStaticStoreRegex::build_document_from_id(uint32_t document_id) {
        Doc document;
        document.title = documents[document_id].title;
        document.start_loc = documents[document_id].start_location;
        return document;
    }
    std::vector<std::string> RecSplitEliasFanoStaticStoreRegex::getWordsFromTrigram(std::string tri) {
