...
// Question greek guy or smth
namespace ModularStoreDeltaEncodedNS {

    class ModularStoreDeltaEncodedDictWord {
    public:
        HashTrie::Node* word;
        uint32_t last_added_id;
        DeltaEncodedArray documents_in;
        std::vector<ModularStoreDeltaEncodedDictWord*> originals;

        ModularStoreDeltaEncodedDictWord(){
            this->word = nullptr;
            documents_in = DeltaEncodedArray{};
            last_added_id = 0;
            originals = std::vector<ModularStoreDeltaEncodedDictWord*>{}; // Fuzzy search not actually implemented
        }
        explicit ModularStoreDeltaEncodedDictWord(HashTrie::Node* node) {
            this->word = node; 
            documents_in = DeltaEncodedArray{};
            last_added_id = 0;
            originals = std::vector<ModularStoreDeltaEncodedDictWord*>{};
        }
    };

    class ModularStoreDeltaEncoded : public IStore {
    private:
        // Words and titles yk
        ...
        CuckooHashingDictionary<std::string, ModularStoreDeltaEncodedDictWord> word_node_store;


    public:
        ModularStoreDeltaEncoded(StringHashFamily* _hasher, int size) : tree(), doc_nodes(100), word_node_store(size, _hasher) {hasher = _hasher;};
        ~ModularStoreDeltaEncoded() override = default;
    ...
    inline void ModularStoreDeltaEncoded::add_document(Doc document) {
    ...
    inline void ModularStoreDeltaEncoded::add(std::string word, Doc document) {
		auto word_node = tree.add(word);
    	ModularStoreDeltaEncodedDictWord* existing = word_node_store.get(word);
    	if (existing) {
    		...
    		ModularStoreDeltaEncodedDictWord pdw(word_node);
    		pdw.documents_in.add(id_for_last_added_doc);
            pdw.last_added_id = id_for_last_added_doc;
    		word_node_store.add(word, std::move(pdw));
    	}
    }
    inline std::vector<Doc> ModularStoreDeltaEncoded::get(std::string word) {
		ModularStoreDeltaEncodedDictWord* tdw = word_node_store.get(word);
    	if (!tdw) return {};
    ...
    inline void ModularStoreDeltaEncoded::map_id_to_doc_nodes(std::vector<Doc> &res, std::vector<uint32_t> &ids) {
    ...
    inline int ModularStoreDeltaEncoded::get_num_docs() {
      return doc_nodes.n;
    }
}
