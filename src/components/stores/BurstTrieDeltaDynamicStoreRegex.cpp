#pragma once
#include "BurstTrieDeltaDynamicStoreRegex.h"


namespace BurstTrieDeltaDynamicStoreRegexNS {
    BurstTrieDeltaDynamicStoreRegex::BurstTrieDeltaDynamicStoreRegex()
        : word_dictionary(),
          postings_lists(),
          documents(2),
          last_added_document_id(0) {}

    void BurstTrieDeltaDynamicStoreRegex::add_document(Doc document) {
    ...
    void BurstTrieDeltaDynamicStoreRegex::add(std::string word, Doc document) {
        if (word.empty()) {
            return;
        }
    ...
    std::vector<Doc> BurstTrieDeltaDynamicStoreRegex::get(std::string word) {
    ...
    int BurstTrieDeltaDynamicStoreRegex::get_num_docs() {
        return documents.n;
    }

    Doc BurstTrieDeltaDynamicStoreRegex::build_document_from_id(uint32_t document_id) {
        Doc document;
        document.title = documents[document_id].title;
        document.start_loc = documents[document_id].start_location;
        return document;
    }
    std::vector<std::string> BurstTrieDeltaDynamicStoreRegex::getWordsFromTrigram(std::string tri) {
